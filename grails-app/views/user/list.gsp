...
<div id="wrap">
    ...
    <div class="header">
        ...
        <div>
            <ul>
                <g:each in="${uwcuser}" var="c" status="i">
                    <li> User ${i + 1} is: <g:link controller="user" action="show" id="${c.username}"> ${c.username} </g:link> </li>
                    <li> Delete <g:link controller="user" action="deleteAccount" id="${c.username}"> ${c.username} </g:link></li>
                </g:each>
            </ul>
        </div>
